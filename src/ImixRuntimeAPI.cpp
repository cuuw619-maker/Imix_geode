...
#else
    // Safe C++ fallback keeps Windows/non-Rust builds functional.
    float offset = candidate == 0 ? -6.f : candidate == 1 ? -3.f : candidate == 3 ? 3.f : candidate >= 4 ? 6.f : 0.f;
    float lead = std::clamp(28.f + offset + std::clamp(speed * .008f, 0.f, 10.f)
                            - std::min(std::fabs(hazardDy) * .045f, 18.f), 12.f, 52.f);
    float alignment = std::fabs(hazardDx - lead);
    return {hazardDx >= 8.f && hazardDx <= 115.f && alignment <= 24.f,
            lead, std::clamp(1.f - alignment / 80.f, .05f, .99f),
            velocityY > 1.f ? 1 : velocityY < -1.f ? 2 : 0};
#endif
}
